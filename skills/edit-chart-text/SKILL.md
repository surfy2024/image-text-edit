...
8. 二次请求必须保持 `replacements` 原顺序，在顶层加入 `confirmation_report_path`，并从同一候选记录原样复制确认字段 `candidate_token`、`candidate_number`、`polygon`（写入 `candidate_polygon`）；候选记录存在 `substring_occurrence` 时也必须原样复制；对应 replacement 设置 `scope=one`。不得拼接来自不同候选的字段，也不得只传编号。
