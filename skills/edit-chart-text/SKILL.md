...
2. 每次从当前请求提取 `old_text`、`new_text`、`scope` 和 `location_hint`，绝不复用示例或历史值。缺少旧文字或新文字时先补齐。
...
7. 退出码 3 时，展示 CLI 返回的 candidates 路径，并读取 CLI 返回的 report 路径。报告的 `run_id`、`source_sha256` 和候选 `candidate_token` 共同绑定这次确认。
8. 二次请求必须保持 `replacements` 原顺序，在顶层加入 `confirmation_report_path`，并从同一候选记录原样复制 `candidate_token`、`candidate_number`、`polygon`（写入 `candidate_polygon`）；对应 replacement 设置 `scope=one`。不得拼接来自不同候选的字段，也不得只传编号。
