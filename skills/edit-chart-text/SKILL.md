...
9. 用户选择编号后，从 `<stem>_edit-report.json` 查找该 `candidate_number` 及其 `replacement_index`。重写请求，在对应 replacement 中设置该 `candidate_number` 与 `scope=one`，再运行命令；多 replacement 时严格按 `replacement_index` 对应。
