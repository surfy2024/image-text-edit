...
4. 仅当用户明确说“全部”时设置 `scope=all`。若文字可能出现多处且用户未说明范围，设置 `scope=ask`。将单处明确目标设置为 `scope=one`。仅将 `location_hint` 用作人工描述和诊断上下文；它不参与自动筛选，也不能代替 polygon 确认。
5. 在输入图片旁写入 UTF-8 JSON 请求文件，包含 `image_path` 和 `replacements`；绝不覆盖原图。相对 `image_path` 始终以请求 JSON 所在目录为基准解析，因此同目录图片只写文件名即可。
...
9. 将 `candidate_number` 仅视为报告中的人类可见编号。用户选择编号后，从 `<stem>_edit-report.json` 的同一条记录复制 `candidate_number`、`polygon` 和 `replacement_index`。重写请求，在对应 replacement 中设置 `scope=one`、该 `candidate_number` 及 `candidate_polygon`，再运行命令。绝不只传编号；重跑时以 polygon 几何指纹确认目标。
